...
const ITEM_TYPES = [
  ...
  'Dramatized Solo Verse', 'Film for Early Years', 'Play',
  'Cultural Creative Dance', 'Modern Creative Dance', 'Narrative',
  'Film', 'Play in Kenyan Sign Language',
  'Dramatized Dance for Special Needs (Mentally Handicapped)',
  'Dramatized Dance for Special Needs (Physically Handicapped)',
];

// Items allowed per category
const CATEGORY_ITEMS: Record<string, string[]> = {
  'Pre-Primary': ['Dramatized Singing Games', 'Dramatized Verse (Solo)', 'Dramatized Verse (Choral)'],
  'Lower Primary': ['Dramatized Verse (Choral)', 'Dramatized Solo Verse', 'Film for Early Years'],
  'Primary': [
    'Play', 'Cultural Creative Dance', 'Modern Creative Dance',
    'Dramatized Verse (Solo)', 'Dramatized Verse (Choral)', 'Narrative', 'Film',
...
function validateSubmission(data: SubmissionRequest): { valid: boolean; error?: string } {
  ...
  const allowedItems = CATEGORY_ITEMS[data.category] || [];
  for (let i = 0; i < data.items.length; i++) {
    const item = data.items[i];
    if (!item.itemType || !ITEM_TYPES.includes(item.itemType)) return { valid: false, error: `Invalid item type for item ${i + 1}` };
    if (!allowedItems.includes(item.itemType)) return { valid: false, error: `Item type "${item.itemType}" is not allowed for ${data.category}` };
    if ((item.itemType === 'Play' || item.itemType === 'Play in Kenyan Sign Language') && item.language && !LANGUAGES.includes(item.language)) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (countError) { console.error('Error checking category count:', countError); }
    else if (categoryCount && categoryCount.length >= 3) {
